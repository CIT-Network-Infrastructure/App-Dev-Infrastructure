...
class FlaskDB:
    ...
    def __init__(self,
        ...
        self.conn_active = True if self.check_connection() else False
    ...
    def check_connection(self, timeout_limit=10) -> bool:
        """
        Checks if the database connection is active by attempting to make a `psycopg2` connection.
        Includes a countdown timer that stops when a connection is successful or fails.
        """
        stop_event = threading.Event()
        countdown_thread = threading.Thread(target=countdown, args=(timeout_limit, stop_event, "Attempting to connect"))
        ...
        try:
            ...
            countdown_thread.join()  # Ensure the countdown thread terminates cleanly
            ...
            countdown(5, threading.Event(), "Exiting program")  # Inline countdown for graceful exit
            stop_program(1)
...
def countdown(seconds, stop_event, message=None):
    """
    A countdown function that runs until the time is up or a stop event is set.
    """
...
class FlaskAppDB:
    ...
    def __init__(
        ...
        :param app_config: An optional (but necessary) dictionary of `app.config` settings
        """
